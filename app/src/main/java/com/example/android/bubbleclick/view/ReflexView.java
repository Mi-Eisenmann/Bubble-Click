package com.example.android.bubbleclick.view;
...
import com.example.android.bubbleclick.R;
...
public class ReflexView extends View {
    ...
    public static final int GETLIFE_SOUND_ID = 4;
    public static final int LOOSELIFE_SOUND_ID = 5;
    public static final int LEVELUP_SOUND_ID = 6;
    ...
    private void initializeSoundEffects(Context context) {
        ...
        soundMap.put(LEVELUP_SOUND_ID, soundPool.load(context, R.raw.dixie_horn_levelup, SOUND_PRIORITY));
        soundMap.put(LOOSELIFE_SOUND_ID, soundPool.load(context, R.raw.kid_laugh_looselife, SOUND_PRIORITY));
        soundMap.put(GETLIFE_SOUND_ID, soundPool.load(context, R.raw.pew_getlife, SOUND_PRIORITY));
    ...
    public void addNewSpot()
    {
        ...
        // coordinates for the random spawning point
        int x3 = random.nextInt(viewWidth - SPOT_DIAMETER);
        int y3 = random.nextInt(viewHeight - SPOT_DIAMETER);
        ...
        // Make a random decision whether the new spot is moving or standing still
        int randomMove = random.nextInt(2);

        if (randomMove >= 1) {
            // configure and start spot's moving animation
            ...
            // configure and start spot's standing animation
            spot.animate().x(x).y(y).scaleX(SCALE_X).scaleY(SCALE_Y)
                    .setDuration(animationTime).setListener(
                    new AnimatorListenerAdapter() {
                        @Override
                        public void onAnimationStart(Animator animation) {
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        if (soundPool != null)
           soundPool.play(LOOSELIFE_SOUND_ID, volume, volume, SOUND_PRIORITY, 0, 1.0F);
    ...
    private void touchedSpot(ImageView spot)
    {
        ...
        if (spotsTouched % NEW_LEVEL == 0) {
            ...
            // Change the background color every level
            int r = random.nextInt(255);
            int g = random.nextInt(255);
            int b = random.nextInt(255);

            int newBackgroundColor = Color.rgb(r,g,b);

            //relativeLayout.setBackgroundColor(Color.RED);
            relativeLayout.setBackgroundColor(newBackgroundColor);

            // play the level-up sounds
            if (soundPool != null)
                soundPool.play(LEVELUP_SOUND_ID, volume, volume,
                        SOUND_PRIORITY, 0, 1f);
    ...
    public void missedSpot(ImageView spot)
    {
        ...
        if (soundPool != null)
            soundPool.play(LOOSELIFE_SOUND_ID, volume, volume,
                    SOUND_PRIORITY, 0, 1f);
